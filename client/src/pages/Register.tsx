...
import { Loader2, Mail, Lock, User, Eye, EyeOff, Ship, Building2 } from "lucide-react";
      ...
      <div className="min-h-screen flex items-center justify-center pt-20 pb-12 px-4">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground font-sans mt-2">Join the BraveMarines community</p>
          </div>
          ...
          <Card className="border-border/50 shadow-xl">
            <CardContent className="p-8">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <button type="button" onClick={() => setAccountType("seafarer")}
                  className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${
                    accountType === "seafarer" ? "border-gold-400 bg-gold-400/5" : "border-border hover:border-gold-400/30"
                  }`}>
                  <Ship className={`h-6 w-6 ${accountType === "seafarer" ? "text-gold-500" : "text-muted-foreground"}`} />
                  <span className={`text-sm font-semibold font-sans ${accountType === "seafarer" ? "text-gold-500" : "text-muted-foreground"}`}>Seafarer</span>
                </button>
                <button type="button" onClick={() => setAccountType("company")}
                  className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${
                    accountType === "company" ? "border-gold-400 bg-gold-400/5" : "border-border hover:border-gold-400/30"
                  }`}>
                  <Building2 className={`h-6 w-6 ${accountType === "company" ? "text-gold-500" : "text-muted-foreground"}`} />
                  <span className={`text-sm font-semibold font-sans ${accountType === "company" ? "text-gold-500" : "text-muted-foreground"}`}>Company</span>
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="name" placeholder="Enter your full name" value={name}
                      onChange={(e) => setName(e.target.value)} className="pl-10 h-12 font-sans" required />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="confirmPassword" className="font-sans font-medium">Confirm Password</Label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <Input id="confirmPassword" type="password" placeholder="Confirm your password"
                      value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-10 h-12 font-sans" required />
                  </div>
                </div>
              ...
              <p className="text-center text-sm text-muted-foreground font-sans mt-6">
                ...
                <Link href="/login" className="text-gold-500 hover:text-gold-600 font-semibold">Sign In</Link>
              </p>
            </CardContent>
          </Card>
        </div>
